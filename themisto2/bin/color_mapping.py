...
def match_assemblies(metadata, assembly_input, assembly_suffix, sample_col, label_col):
    ...
    if assembly_input_path.is_dir():
        ...
        no_assembly_by_group = metadata.loc[no_assembly, label_col].astype(str).value_counts().to_dict()
...
def main():
    ...
    metadata = metadata.sort_values([args.label_col, args.sample_col], key=lambda col: col.astype(str))
    ...
    metadata, no_assembly_by_group, on_disk_no_metadata = match_assemblies(
        metadata, assembly_input, args.assembly_suffix, args.sample_col, args.label_col
    )
