...
- `--max_contamination` (Maximum bin contamination threshold %, default: 5)
- `--checkm2_db` (Checkm2 database path default: `/data/pam/software/checkm2_db/uniref100.KO.1.dmnd`)
- `--checkm1` (Use Checkm, default: false, uses CheckM2)
- `--lock_phred` (set PHRED quality offset to 33 when using SPAdes default: false)


### Inputs
- Set of refined bins generated by the tool [Binning_refiner](https://github.com/songweizhi/Binning_refiner)
...
### Dependencies
All dependencies are contained in docker containers (which are pulled/built at runtime).
The image used for BWA, also bundles samtools (v1.21) to facilitate conversion of BWA alignments to BAM format, reducing disk footprint.
