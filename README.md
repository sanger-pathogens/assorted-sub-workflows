...
### Only downloading data/running analyses that haven't been done previously

When attempting to download files from iRODS, the `IRODS_EXTRACTOR` subworkflow and other components of the broader `MIXED_INPUT` subworkflow checks if the expected ouput fastq files exist in the output reads folder (default in `preprocessed_reads/`, but can be changed e.g. to `raw_fastq/`) under the main `results/` folder for each sample e.g. in `results/12345_1#67/preprocessed_reads/12345_1#67_1.fastq.gz`. If files with matching name root are present, it then skips download and any further processing for this sample.  
The idea behind this mechanism is to only update the results of a previous run with the increment of new input data that may have been made available at the source. For instance, every so often new data files may become available that match a given query to iRODS or the ENA, typically as new sequencing runs are completed as part of an ongoing study. To access those reads and generate any downstream analysis, there is no need to know the specifics of the new data and to selectively download them. Instead, the same whole study can be requested again and `IRODS_EXTRACTOR`, will know to only update the results folder rather than re-downloading and re-processing everything - that is if you keep the same results folder and have the right values set for the relevant parameters: `preexisting_output_dir` and `existing_output_id_suffix`, see help message for details; default behaviour is to update.  

> **_NOTE:_** Currently, the filter on what to download from iRODS via Beefeater based on already existing output files doesn't work with other platforms that Illumina i.e. recurring pipeline runs with the same iRODS query parameters for ONT sequence data may redownload the same files again. 
...
This logic was originally designed for the `IRODS_EXTRACTOR` subworkflow (read download from iRODS) but has recently been extended to the broader `MIXED_INPUT` subworkflow, a generic interface which delivers reads from multiple sources as input to various pipelines. These sources include: iRODS (data queried/downloaded through `IRODS_EXTRACTOR`), the ENA (data queried/downloaded through `ENA_DOWNLOAD`), a folder (input data collated into a manifest from a folder scan with `MANIFEST_FROM_DIR`). Similarly to studies on iRODS, ENA studies can be incremnetd and a given query may yield more data, which may be use dto update the `results/` folder. Likewise, a folder used for collaborators/another pipeline to dump data may be supplied with new files on a regular basis and that increment be selectively used as input to update the `results/` folder. 

Finally, this mechanism has been designed to be applied to any form of input/output, as long as these can be matched based on the sample ID, which is inferred from the root of the name of specific output files. For instance, instead of fastq reads files, fasta assembly files may be considered, or GFF3 annotation files. This logic has thus been used to enable pipeline chaining, whereby the output of a pipeline is dropped off in a given folder to be used by another pipeline scanning that same folder for input.

When used in the context of an analysis pipeline, the user may want to think about which stage of the pipeline they want to have as an effective checkpoint and set `preexisting_output_dir` and `existing_output_id_suffix` accordingly. For instance, it may not be adviseable to choose the output published by the final stage of a long and complex workflow as the files to look out for, as many input samples may fail during the workflow and never produce such output e.g. due to bad quality or too large data. If so, automated pipeline chains may result in systematic recomputation of anlyses bound to fail. Future developments may make possible to provide a list of folders/id suffixes that characterise sevral file types that can all act as markers of a given input sample needing to be skipped; for instance, failure reports may be produced and used to disable repeat analysis.  
