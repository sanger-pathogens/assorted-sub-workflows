...
def argparser():
    ...
    parser.add_argument("-g", "--gap_character", default="N",
                    help="character to use for Gaps")
...
def calculate_gaps_to_add(gap_start_position, gap_end_position, gap_character):
    return [gap_character] * (gap_end_position - gap_start_position)

def filtered_bcf_to_fasta(filtered_bcf_file, reference_lengths, gap_character):
    ...
    with VariantFile(filtered_bcf_file) as vcf_reader:
        for record in vcf_reader.fetch():
            ...
            if record.pos == previous_positions[record_chrom]: # Insertion - remove previous character and add 'N'
                ...
                if previous_positions[record_chrom] + 1 < record.pos: # large deletion add gaps before adding next position
                    sequences[record_chrom].extend(calculate_gaps_to_add(previous_positions[record_chrom] + 1, record.pos), gap_character)
        ...
        for chrom in sequences:
            if len(sequences[chrom]) != reference_lengths[record_chrom]: # if gap at the end
                sequences[chrom].extend(calculate_gaps_to_add(len(sequences[chrom]), reference_lengths[chrom]), gap_character)
        return sequences
...
if __name__ == '__main__':
    ...
    pseudogenome_sequence_lists = filtered_bcf_to_fasta(args.filtered_bcf_file, reference_lengths, args.gap_character)
