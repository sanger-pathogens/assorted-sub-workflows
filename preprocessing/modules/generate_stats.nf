process GENERATE_STATS {
    ...
    input:
    tuple val(meta), path("trimmed_read_1.fq"), path("trimmed_read_2.fq"), path("clean_read_1.fq"), path("clean_read_2.fq"), path("host_read_1.fq.gz"), path("host_read_2.fq.gz"), path("original_read_1.fq"), path("original_read_2.fq")
    ...
    clean_1_reads=\$((`cat clean_read_1.fq | wc -l` / 4))
    clean_2_reads=\$((`cat clean_read_2.fq | wc -l` / 4))
    host_1_reads=\$((`zcat host_read_1.fq.gz | wc -l` / 4))
    host_2_reads=\$((`zcat host_read_2.fq.gz | wc -l` / 4))
    original_1_reads=\$((`cat original_read_1.fq | wc -l` / 4))
    original_2_reads=\$((`cat original_read_2.fq | wc -l` / 4))
