...
def read_config(path: Path) -> dict:
    ...
    try:
        ...
        logging.error(f"Invalid config structure detected in configuration file {path}: {e}")
    return config
...
def validate_config(config: dict) -> None:
    ...
    if expected_tools == set():
        raise InvalidConfig(f"Config does not contain at least one valid tool: {valid_tools}")
    if unexpected_tools != set():
        logging.warning(f"Config contains keys that were not expected (and are ignored): {unexpected_tools}")
    for tool in expected_tools:
        missing_keys = {"id_column", "keep_columns"} - config[tool].keys()
        if missing_keys != set():
            raise InvalidConfig(f"Config is missing a mandatory key for tool '{tool}': {missing_keys}")
...
def process_input_report(df: pd.DataFrame, config: dict, tool: str, qc_stage: Optional[str] = None) -> pd.DataFrame:
    ...
    if qc_stage is not None:
        ...
        rename_dict[config[tool.upper()]["id_column"]] = f"{qc_stage}_{JOIN_COLUMN_STEM}"
    else:
...
def parse_args():
    ...
    parser.add_argument('--pre_qc_checkm2', type=Path, required=True, help='Path to pre-QC CheckM2 TSV')
    parser.add_argument('--pre_qc_gunc', type=Path, required=True, help='Path to pre-QC GUNC TSV')
    parser.add_argument('--post_qc_checkm2', type=Path, required=True, help='Path to post-QC CheckM2 TSV')
    parser.add_argument('--post_qc_gunc', type=Path, required=True, help='Path to post-QC GUNC TSV')
    parser.add_argument('--gtdbtk', type=Path, required=True, help='Path to GTDBTK TSV')
    parser.add_argument('--config', type=Path, required=True, help='Path to report configuration file')
...
def main():
    ...
    for arg, qc_stage in args_qc_stage.items():
        ...
        df = process_input_report(read_tsv(vars(args)[arg], arg), config, tool, qc_stage)
        if qc_stage == "postqc":
