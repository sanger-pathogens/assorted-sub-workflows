...
It is possible to add filtering rules to the autoqc config for columns\* within the report config. If using a custom report config you can customise your AutoQC config: for the column using the tool name (lowercase) and keep*column from the report config seperated by an underscore. i.e. `<tool>*<keep*column>`. Note that for adding filters based on CheckM2 or GUNC you must specify whether to use preqc or postqc reports i.e. `<tool>*<qc*stage>*<keep_column>`, as shown in the filterable column list above.
