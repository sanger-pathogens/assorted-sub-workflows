process REMOVE_SMALL_CONTIGS {
    ...
    script:
    command = "${projectDir}/assorted-sub-workflows/generate_mags/bin/rm_short_contigs.py"
...
process FIX_MEGAHIT_CONTIG_NAMING {
    ...
    script:
    command = "${projectDir}/assorted-sub-workflows/generate_mags/bin/fix_megahit_contig_naming.py"
    long_scaffolds = "${meta.ID}_long.scaffolds"
    """
    ${command} ${params.min_contig} ${contigs} > ${long_scaffolds}
    """
...
process SORT_CONTIGS {
    ...
    script:
    command = "${projectDir}/assorted-sub-workflows/generate_mags/bin/sort_contigs.py"
